...
namespace Rock.Blocks.Dar
{
    ...
    public class CybersourceDonationEntry : RockObsidianBlockType
    {
        ...
        public override object GetObsidianBlockInitialization()
        {
            ...
            if ( currentPerson == null )
            {
                return new InitBag
                {
                    ...
                    mode = GetMode(),
                    accounts = new List<AccountOptionBag>(),
        ...
        public BlockActionResult ProcessPayment( ProcessPaymentRequestBag bag )
        {
            ...
            using ( var rockContext = new RockContext() )
            {
                ...
                if ( account == null )
                {
                    return ActionBadRequest( "La cuenta no esta permitida en este bloque." );
                }
                ...
                catch ( Exception ex )
                {
                    ...
                    {
                        success = false,
                        message = $"Cobro aprobado en Cybersource, pero fallo registro en Financial: {ex.Message}",
                        responseCode = cybsResult.responseCode,
                        authorizationNumber = cybsResult.authorizationNumber,
                        referenceNumber = cybsResult.referenceNumber,
                        auditNumber = cybsResult.auditNumber,
        ...
        private int SaveFinancialTransaction(
            ...
            string mode )
        {
            var personAliasId = new PersonAliasService( rockContext ).GetPrimaryAliasId( person.Id );
            if ( !personAliasId.HasValue )
            {
                throw new Exception( "No se encontro alias principal para la persona." );
            }
            ...
            var transaction = new FinancialTransaction
            {
                ...
                Summary = BuildSummary( bag.note, chargeResult.referenceNumber, chargeResult.authorizationNumber ),
                Status = "Approved",
...
ORDER BY
    ft.TransactionDateTime DESC";
        ...
        private string ValidatePaymentRequest( ProcessPaymentRequestBag bag )
        {
            ...
            {
                return "Moneda invalida. Valores permitidos: GTQ o USD.";
            }
        ...
        private CybersourceChargeResult ChargeWithCybersource( ProcessPaymentRequestBag bag, CybersourceGatewaySettings settings )
        {
            ...
            if ( !Uri.TryCreate( url, UriKind.Absolute, out var requestUri ) )
            {
                return new CybersourceChargeResult
                {
                    ok = false,
                    responseCode = "CONFIG",
                    errorMessage = "Base URL invalido para Cybersource."
                };
            }
            ...
            if ( requestHost.IsNullOrWhiteSpace() )
            {
                return new CybersourceChargeResult
                {
                    ok = false,
                    responseCode = "CONFIG",
                    errorMessage = "Host invalido para Cybersource."
                };
            }
            ...
            foreach ( var profile in profiles )
            {
                ...
                catch ( Exception ex )
                {
                    return new CybersourceChargeResult
                    {
                        ok = false,
                        responseCode = "CONFIG",
                        errorMessage = "Shared secret invalido: " + ex.Message
                    };
                }
            ...
            return lastUnauthorizedResult ?? new CybersourceChargeResult
            {
                ok = false,
                responseCode = "401",
                errorMessage = "Cybersource rechazo el pago (HTTP 401)."
            };
        }
            ...
            CybersourceSignatureProfile profile )
        {
            ...
            var defaultErrorMessage = string.Format(
                ...
                "Cybersource rechazo el pago (HTTP {0}). Firma usada: {1}/{2}. Verifica mode/host/merchantId/keyId/shared secret.",
                statusCode,
                profile.dateHeaderName,
                profile.requestTargetHeaderName );
