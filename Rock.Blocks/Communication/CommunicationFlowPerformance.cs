...
namespace Rock.Blocks.Communication
{
    ...
    public class CommunicationFlowPerformance : RockBlockType, IBreadCrumbBlock
    {
        ...
        private PerformanceBag GetFlowPerformance( RockContext rockContext, string flowKey, bool allowIntegerIds )
        {
            ...
            var ctx = new CommunicationFlowPerformanceService( rockContext );
            var scheduledTriggerTypes = new[] { CommunicationFlowTriggerType.OneTime, CommunicationFlowTriggerType.Recurring };
            var ( communicationFlowPerformanceBag, communicationFlowId ) = ctx.CommunicationFlows
                ...
                .Select( cf => new
                {
                    ...
                    cf.Name,
                    cf.Schedule,
                    CommunicationCount = cf.CommunicationFlowCommunications.Count
                })
                .ToList()
                .Select( cf =>
                    (
                        new PerformanceBag
                        {
                            ...
                            CommunicationFlowStartDateTime = scheduledTriggerTypes.Contains( cf.TriggerType )
                                ? ( cf.Schedule?.FirstStartDateTime ?? cf.Schedule?.EffectiveStartDate )
                                : null,
                            CommunicationCount = cf.CommunicationCount
                        },
            ...
            communicationFlowPerformanceBag.Messages = ctx.CommunicationFlowInstanceCommunications
                ...
                .SelectMany( ic => ic.Communication.Recipients
                    .Select( cr => new
                    {
                        ...
                        Converted =
                            ctx.CommunicationFlowInstanceCommunicationConversions
                               .Where( cficc =>
                                       cficc.CommunicationFlowInstanceCommunicationId == ic.Id
                                       && cficc.PersonAliasId == cr.PersonAliasId )
                               .OrderBy( cficc => cficc.Date )
                               .Select( cficc => ( DateTime? ) cficc.Date )
                               .FirstOrDefault()
                    } )
                )
                .ToList()
            ...
            communicationFlowPerformanceBag.Instances = ctx.CommunicationFlowInstances
        ...
        private class CommunicationFlowPerformanceService
        {
            private readonly RockContext _rockContext;

            public CommunicationFlowPerformanceService( RockContext rockContext )
            {
            ...
            public IQueryable<CommunicationFlowInstanceCommunicationConversion> CommunicationFlowInstanceCommunicationConversions
            {
                get
                {
                    return new CommunicationFlowInstanceCommunicationConversionService( _rockContext ).Queryable();
                }
            }
