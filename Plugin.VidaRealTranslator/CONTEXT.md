...
| `ExcludedSites` | Memo | (vacío) | Sitios donde NUNCA inyectar el script (nombre o Id, uno por línea). |
...
- **Grids: dos markups distintos (1.4.6).** WebForms usa `<table class="grid-table">` con `<td>`; Obsidian (v15+) usa divs `.grid-row`/`.grid-cell`. `DATA_CELLS` cubre AMBOS. Si Rock cambia otra vez el markup del grid, esta exclusión se cae en silencio y todo el contenido de los grids vuelve a irse a la IA — es el fallo más caro que ha tenido el plugin. Verificarlo en cada upgrade.
- **Contadores en vivo y el bucle de `sendBatch` (1.4.6).** `sendBatch()` llama `run(document.body)` al recibir traducciones nuevas; cualquier texto que cambie solo se AUTOALIMENTA a través de ese rescan. El `churning()` por ranura es la defensa genérica (3 textos distintos en 8 s → ranura apagada). Si aparece un falso positivo (una ranura de UI legítima que rota mucho texto en segundos), subir `SLOT_MAX_CHANGES`.
- **Frases del kiosco con el nombre interpolado (1.4.6, SIN solución dentro del plugin).** `"Select the group you would like to check <nickName> into"` y `"Congratulations <nickName>!"` son UN nodo de texto: UI y PII inseparables. Hoy se sacrifica la frase (queda en inglés). La única solución real es i18n en el `.obs` del kiosco — fuera del alcance de un traductor DOM.
